...
def local_channel_validator():
    ...
    with patch("pixi_ros.validator.get_index"), patch(
        ...
        # Ensure conda-forge check never returns True so we reach the custom channel step
...
def test_custom_channel_relative_path_resolves(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("pixi_ros.validator.RosDistroValidator.check_conda_forge_availability", return_value=False),
    ):
        mock_dist = MagicMock()
        mock_dist.release_packages = {}
        mock_distro.return_value = mock_dist
